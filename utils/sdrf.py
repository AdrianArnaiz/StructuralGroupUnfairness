...
def sdrf(
    ...
    added_links = []
    edge_index = GW.edgelist
    if is_undirected:
        edge_index = to_undirected(edge_index)
    A = to_dense_adj(remove_self_loops(edge_index)[0])[0]
    N = A.shape[0]
    #G = to_networkx(Data(edge_index=GW.edgelist, num_nodes=GW.num_nodes))
    #if is_undirected:
    #    G = G.to_undirected()
    A = A.cuda()
    C = torch.zeros(N, N).cuda()
    ...
    for x in tqdm(range(loops)):
        ...
        if is_undirected:
            x_neighbors = GW.edgelist[1, GW.edgelist[0] == x].tolist() + [x] #list(G.neighbors(x)) + [x]
            y_neighbors = GW.edgelist[1, GW.edgelist[0] == y].tolist() + [y] #list(G.neighbors(y)) + [y]
        else:
            x_neighbors = GW.edgelist[1, GW.edgelist[0] == x].tolist() + [x] #list(G.successors(x)) + [x]
            y_neighbors = GW.edgelist[1, GW.edgelist[0] == y].tolist() + [y] #list(G.predecessors(y)) + [y]
        candidates = []
        for i in x_neighbors:
            for j in y_neighbors:
                if (i != j) and (not GW.is_edge(i, j)):
        ...
        if len(candidates):
            ...
            k, l = candidates[np.argmax(improvements)] 
            """candidates[
                np.random.choice(
...
def sdrf_nx(
    ...
    return from_networkx(G), added_links
